...
try:
    ...
    st.error(
        "CRITICAL ERROR (app_streamlit.py): app_config.py not found. Ensure it's in PYTHONPATH."
    )
    st.stop()

# --- Refactored Module Imports ---
try:
    import kpi_management.groups as kpi_groups_manager
    import kpi_management.subgroups as kpi_subgroups_manager
    import kpi_management.indicators as kpi_indicators_manager
    import kpi_management.specs as kpi_specs_manager
    import kpi_management.templates as kpi_templates_manager
    import kpi_management.links as kpi_links_manager
    import stabilimenti_management.crud as stabilimenti_manager
    import target_management.annual as annual_targets_manager
    import data_retriever as dr
    import export_manager
    from db_core.setup import setup_databases
except ImportError as e:
    st.error(
        f"CRITICAL ERROR (app_streamlit.py): Failed to import one or more refactored modules: {e}"
    )
    st.error(
        "Ensure all new modules (db_core, kpi_management, etc.) and their __init__.py files are correctly placed."
    )
    st.error(traceback.format_exc())
    st.stop()

# --- FIX: Call st.set_page_config() IMMEDIATELY after imports ---
...
# --- Target Tab Callbacks (defined before UI to be accessible) ---
def on_master_target_ui_change_st(master_id, target_num_str, key_suffix=""):
    ...
    try:
        ...
        if not (master_backing_data and master_backing_data.get("is_master_kpi")):
            return

        master_widget_key = f"kpi_entry_{master_id}_target{target_num}{key_suffix}"
        master_target_val = st.session_state.get(
            master_widget_key, master_backing_data.get(f"target{target_num}", 0.0)
        )
        master_backing_data[f"target{target_num}"] = float(master_target_val)

        sum_manual_or_formula_sub_targets = 0.0
        non_manual_subs_for_dist, total_weight_for_dist = [], 0.0

        for sub_info in master_backing_data.get("sub_kpis_with_weights", []):
            sub_id, sub_weight = sub_info["sub_kpi_spec_id"], sub_info.get(
                "weight", 1.0
            )
            ...
            if sub_backing_data and sub_backing_data.get("is_sub_kpi"):
                ...
                sub_use_formula = st.session_state.get(
                    f"{sub_widget_key_base}_use_formula{target_num}{key_suffix}",
                    sub_backing_data.get(f"target{target_num}_is_formula_based", False),
                )
                sub_is_manual = st.session_state.get(
                    f"{sub_widget_key_base}_is_manual{target_num}{key_suffix}",
                    sub_backing_data.get(f"is_manual{target_num}", True),
                )
                sub_target_val = st.session_state.get(
                    f"{sub_widget_key_base}_target{target_num}{key_suffix}",
                    sub_backing_data.get(f"target{target_num}", 0.0),
                )

                if sub_use_formula or sub_is_manual:
                    sum_manual_or_formula_sub_targets += float(sub_target_val)
                else:
        ...
        remaining_target = float(master_target_val) - sum_manual_or_formula_sub_targets

        for sub_to_update in non_manual_subs_for_dist:
            value_for_sub = 0.0
            if total_weight_for_dist > 1e-9:
                value_for_sub = (
                    sub_to_update["weight"] / total_weight_for_dist
                ) * remaining_target
            elif remaining_target != 0 and len(non_manual_subs_for_dist) > 0:
                value_for_sub = remaining_target / len(non_manual_subs_for_dist)

            sub_target_key = (
                f"kpi_entry_{sub_to_update['id']}_target{target_num}{key_suffix}"
            )
            st.session_state[sub_target_key] = round(value_for_sub, 2)
            if sub_to_update["id"] in st.session_state.kpi_target_inputs:
                st.session_state.kpi_target_inputs[sub_to_update["id"]][
                    f"target{target_num}"
                ] = round(value_for_sub, 2)
    finally:
...
def on_sub_manual_flag_ui_change_st(sub_id, target_num_str, key_suffix=""):
    ...
    if not (
        sub_backing_data
        and sub_backing_data.get("is_sub_kpi")
        and sub_backing_data.get("master_kpi_id")
    ):
        return

    use_formula_key = f"kpi_entry_{sub_id}_use_formula{target_num}{key_suffix}"
    if st.session_state.get(use_formula_key, False):
        manual_key_self = f"kpi_entry_{sub_id}_is_manual{target_num}{key_suffix}"
        if manual_key_self in st.session_state:
            st.session_state[manual_key_self] = False
        if sub_id in st.session_state.kpi_target_inputs:
            st.session_state.kpi_target_inputs[sub_id][f"is_manual{target_num}"] = False
        return

    manual_flag_key = f"kpi_entry_{sub_id}_is_manual{target_num}{key_suffix}"
    if sub_id in st.session_state.kpi_target_inputs:
        st.session_state.kpi_target_inputs[sub_id][f"is_manual{target_num}"] = (
            st.session_state.get(manual_flag_key, True)
        )

    on_master_target_ui_change_st(
        sub_backing_data["master_kpi_id"], str(target_num), key_suffix
    )
...
def _on_use_formula_toggle_st(kpi_id, target_num_str, key_suffix=""):
    ...
    if kpi_id in st.session_state.kpi_target_inputs:
        ...
        if is_using_formula:
            st.session_state.kpi_target_inputs[kpi_id][f"is_manual{target_num}"] = False
            if is_manual_key in st.session_state:
                st.session_state[is_manual_key] = False
        elif st.session_state.kpi_target_inputs[kpi_id].get("is_sub_kpi"):
            on_sub_manual_flag_ui_change_st(kpi_id, str(target_num), key_suffix)
...
def initial_master_sub_ui_distribution_st(key_suffix=""):
    ...
    st.session_state._master_sub_update_active_st = True
    try:
        for kpi_id, data in st.session_state.kpi_target_inputs.items():
            if data.get("is_master_kpi"):
                on_master_target_ui_change_st(kpi_id, "1", key_suffix)
                on_master_target_ui_change_st(kpi_id, "2", key_suffix)
    finally:
        st.session_state._master_sub_update_active_st = False
...
def load_kpi_data_for_target_entry():
    """
    Loads all necessary data for the Target Entry tab into st.session_state.kpi_target_inputs.
    This function reads the currently selected filters (year, stabilimento) and populates a
    dictionary with detailed information for every visible KPI, including any existing
    target data for the selected context.
    """
    st.session_state.kpi_target_inputs = {}  # Reset the backing store on each load
    ...
    # Exit if essential filters are not set
    if not year_str or not stab_name:
        return

    try:
        year = int(year_str)
        # Ensure the stabilimento name-to-ID map is available
        if (
            "stabilimenti_map_target_ui" not in globals()
            or not globals()["stabilimenti_map_target_ui"]
        ):
            globals()["stabilimenti_map_target_ui"] = {
                s["name"]: s["id"] for s in dr.get_all_stabilimenti(only_visible=True)
            }

        stabilimento_id = stabilimenti_map_target_ui.get(stab_name)
        if stabilimento_id is None:
            st.warning(f"ID Stabilimento non trovato per il nome '{stab_name}'.")
            return
    except (ValueError, TypeError) as e:
        st.error(f"Errore nella preparazione dei filtri: {e}")
        return

    # Fetch all visible KPIs to build the UI
    kpis_for_entry = dr.get_all_kpis_detailed(only_visible=True)
    if not kpis_for_entry:
        return  # No visible KPIs to show

    # Loop through each KPI to build its data structure in session_state
    for kpi_row_sqlite in kpis_for_entry:
        kpi_row = dict(kpi_row_sqlite)  # Convert sqlite3.Row to a standard dictionary
        kpi_spec_id = kpi_row.get("id")
        if kpi_spec_id is None:
            continue

        # Fetch existing target data for this specific KPI, year, and stabilimento
        target_row = dr.get_annual_target_entry(year, stabilimento_id, kpi_spec_id)
        existing_target = (
            dict(target_row) if target_row else {}
        )  # Convert to dict or use empty dict

        # Determine the role of the KPI (Master, Sub, or None)
        role = dr.get_kpi_role_details(kpi_spec_id)

        # Determine if targets are formula-based
        is_formula1 = bool(existing_target.get("target1_is_formula_based", False))
        is_formula2 = bool(existing_target.get("target2_is_formula_based", False))

        # Build the comprehensive dictionary for this KPI and store it in session_state
        st.session_state.kpi_target_inputs[kpi_spec_id] = {
            "target1": float(existing_target.get("annual_target1", 0.0) or 0.0),
            "target2": float(existing_target.get("annual_target2", 0.0) or 0.0),
            "is_manual1": (
                bool(existing_target.get("is_target1_manual", True))
                if not is_formula1
                else False
            ),
            "is_manual2": (
                bool(existing_target.get("is_target2_manual", True))
                if not is_formula2
                else False
            ),
            "profile": existing_target.get(
                "distribution_profile", PROFILE_ANNUAL_PROGRESSIVE
            ),
            "logic": existing_target.get("repartition_logic", REPARTITION_LOGIC_ANNO),
            "repartition_values_raw": json.loads(
                existing_target.get("repartition_values", "{}") or "{}"
            ),
            "profile_params_raw": json.loads(
                existing_target.get("profile_params", "{}") or "{}"
            ),
            "calc_type": kpi_row.get("calculation_type"),
            "unit_of_measure": kpi_row.get("unit_of_measure", ""),
            "is_sub_kpi": role["role"] == "sub",
            "master_kpi_id": role.get("master_id"),
            "is_master_kpi": role["role"] == "master",
            "sub_kpis_with_weights": [],  # Will be populated below if it's a master
            "display_name": get_kpi_display_name(kpi_row),
            "target1_is_formula_based": is_formula1,
            "target1_formula": existing_target.get("target1_formula", ""),
            "target1_formula_inputs_json": existing_target.get(
                "target1_formula_inputs", "[]"
            ),
            "target2_is_formula_based": is_formula2,
            "target2_formula": existing_target.get("target2_formula", ""),
            "target2_formula_inputs_json": existing_target.get(
                "target2_formula_inputs", "[]"
            ),
        }

        # If the KPI is a master, fetch its sub-KPIs and their weights
        if role["role"] == "master":
            # This requires a direct DB connection for now as data_retriever might not have this exact function
            try:
                ...
                st.error(
                    f"Errore nel recuperare i pesi dei sub-kpi per il master {kpi_spec_id}: {e}"
                )

    # After loading all data, trigger the initial UI distribution calculation
    key_suffix = f"_{year}_{stabilimento_id}"
    initial_master_sub_ui_distribution_st(key_suffix)


# --- Streamlit Startup & State Initialization ---
try:
    setup_databases()
except Exception as e:
...
if "initialized" not in st.session_state:
    ...
    st.session_state.target_year_sb_filters = str(datetime.datetime.now().year)

    try:
        stabilimenti_for_init = dr.get_all_stabilimenti(only_visible=True)
        st.session_state.target_stab_sb_filters = (
            stabilimenti_for_init[0]["name"] if stabilimenti_for_init else None
        )
    except Exception as e_init:
        st.error(f"Failed to fetch stabilimenti on init: {e_init}")
        st.session_state.target_stab_sb_filters = None
    ...
    st.session_state.results_target_numbers_ms = [1, 2]
...
with tab_stabilimenti:
    ...
    with col1_stab:
        ...
        if stabilimenti_data_raw:
            ...
            st.session_state.df_stabilimenti_original_for_selection = df_stabilimenti
            if not df_stabilimenti.empty:
                display_cols_stab = ["id", "name"]
                ...
                if selection_info_stab and selection_info_stab["selection"]["rows"]:
                    selected_df_index = selection_info_stab["selection"]["rows"][0]
                    if (
                        ...
                        current_editing_stab_data = st.session_state.get(
                            "editing_stabilimento"
                        )
                        current_editing_id = (
                            current_editing_stab_data.get("id")
                            if isinstance(current_editing_stab_data, dict)
                            else None
                        )
                        if current_editing_id != selected_item_data["id"]:
                            st.session_state.editing_stabilimento = selected_item_data
                elif (
                    selection_info_stab and not selection_info_stab["selection"]["rows"]
                ):
    ...
    with col2_stab:
        ...
        with st.form(key="stabilimento_master_form_st", clear_on_submit=False):
            stab_name_input_val = st.text_input(
                ...
                key="stab_name_input_field_st",
            )
            stab_description_input_val = st.text_area(
                "Descrizione",
                value=initial_description_stab,
                key="stab_description_input_field_st",
                height=100,
            )
            stab_visible_input_val = st.checkbox(
                "Visibile",
            ...
            if submitted_stabilimento_form:
                if not stab_name_input_val.strip():
                    ...
                    try:
                        if editing_id_stab is not None:
                            stabilimenti_manager.update_stabilimento(
                                editing_id_stab,
                                stab_name_input_val.strip(),
                                stab_description_input_val.strip(),
                                stab_visible_input_val,
                            )
                            st.success(
                                f"Stabilimento '{stab_name_input_val.strip()}' aggiornato."
                            )
                        else:
                            stabilimenti_manager.add_stabilimento(
                                stab_name_input_val.strip(),
                                stab_description_input_val.strip(),
                                stab_visible_input_val,
                            )
                            st.success(
                                f"Stabilimento '{stab_name_input_val.strip()}' aggiunto."
                            )
                        st.session_state.editing_stabilimento = None
                        st.rerun()
                    except sqlite3.IntegrityError:
                        st.error(
                            f"Errore: Uno stabilimento con nome '{stab_name_input_val.strip()}' esiste già."
                        )
                    except Exception as e:
...
with tab_hierarchy:
    ...
    with col1_hier:
        ...
        if selected_group_name_hier:
            st.session_state.selected_group_id_hier = groups_map[
        ...
        with st.expander("Gestisci Gruppi"):
            with st.form("group_form_hier_st", clear_on_submit=True):
            ...
            if st.session_state.selected_group_id_hier:
                ...
                with st.form(
                    f"edit_group_form_{st.session_state.selected_group_id_hier}_st"
                ):
                    edited_group_name = st.text_input(
                        "Nuovo Nome", value=selected_group_name_hier
                    )
                    ...
                    if (
                        ...
                        try:
                            kpi_groups_manager.update_kpi_group(
                                st.session_state.selected_group_id_hier,
                                edited_group_name,
                            )
                            st.success(f"Gruppo rinominato.")
                            st.session_state.selected_group_id_hier = None
                            st.rerun()
                        except Exception as e:
                ...
                if st.button(
                    "Elimina Gruppo",
                    type="primary",
                    key=f"del_group_{st.session_state.selected_group_id_hier}_st",
                ):
                    try:
                        kpi_groups_manager.delete_kpi_group(
                            st.session_state.selected_group_id_hier
                        )
    ...
    with col2_hier:
        ...
        if selected_subgroup_display_name_hier:
            st.session_state.selected_subgroup_id_hier = subgroups_map[
                selected_subgroup_display_name_hier
            ]
            current_subgroup_details_hier = (
                next(
                    (
                        sg
                        for sg in subgroups
        ...
        with st.expander("Gestisci Sottogruppi"):
            if st.session_state.selected_group_id_hier:
                ...
                templates_map_hier = {
                    "(Nessuno)": None,
                    **{tpl["name"]: tpl["id"] for tpl in templates},
                }
                with st.form("subgroup_form_hier_st", clear_on_submit=True):
                    new_subgroup_name = st.text_input("Nome Nuovo Sottogruppo")
                    selected_template_name_for_new_sg = st.selectbox(
                        "Template", list(templates_map_hier.keys())
                    )
                    ...
                    if add_subgroup_submitted and new_subgroup_name:
                        try:
                            ...
                            kpi_subgroups_manager.add_kpi_subgroup(
                                new_subgroup_name,
                                st.session_state.selected_group_id_hier,
                ...
                if current_subgroup_details_hier:
                    st.markdown("---")
                    st.write(
                        f"**Modifica/Elimina: {current_subgroup_details_hier['name']}**"
                    )
                    with st.form(
                        f"edit_subgroup_form_{current_subgroup_details_hier['id']}_st"
                    ):
                        edited_subgroup_name = st.text_input(
                            "Nuovo Nome", value=current_subgroup_details_hier["name"]
                        )
                        ...
                        if update_subgroup_submitted:
                            ...
                            if (
                                ...
                                != current_subgroup_details_hier["name"]
                                or new_template_id_for_edit != current_tpl_id
                            ):
                                try:
                                    kpi_subgroups_manager.update_kpi_subgroup(
                    ...
                    if st.button(
                        "Elimina Sottogruppo",
                        type="primary",
                        key=f"del_subg_{current_subgroup_details_hier['id']}_st",
                    ):
                        try:
                            kpi_subgroups_manager.delete_kpi_subgroup(
                                current_subgroup_details_hier["id"]
                            )
                            st.success(
        ...
        with st.expander("Gestisci Indicatori"):
            if st.session_state.selected_subgroup_id_hier:
                if is_templated_subgroup:
                    ...
                    with st.form("indicator_form_hier_st", clear_on_submit=True):
                        ...
                        if add_indicator_submitted and new_indicator_name:
                            try:
                                kpi_indicators_manager.add_kpi_indicator(
                                    new_indicator_name,
                    ...
                    if st.session_state.selected_indicator_id_hier:
                        st.markdown("---")
                        st.write(
                            f"**Modifica/Elimina: {selected_indicator_name_hier}**"
                        )
                        with st.form(
                            f"edit_indicator_form_{st.session_state.selected_indicator_id_hier}_st"
                        ):
                            edited_indicator_name = st.text_input(
                                "Nuovo Nome", value=selected_indicator_name_hier
                            )
                            ...
                            if (
                                ...
                                try:
                                    kpi_indicators_manager.update_kpi_indicator(
                                        st.session_state.selected_indicator_id_hier,
                                        edited_indicator_name,
                                        st.session_state.selected_subgroup_id_hier,
                                    )
                                    st.success(f"Indicatore rinominato.")
                                    st.session_state.selected_indicator_id_hier = None
                                    st.rerun()
                                except Exception as e:
                        ...
                        if st.button(
                            "Elimina Indicatore",
                            type="primary",
                            key=f"del_ind_{st.session_state.selected_indicator_id_hier}_st",
                        ):
                            try:
                                kpi_indicators_manager.delete_kpi_indicator(
                                    st.session_state.selected_indicator_id_hier
                                )
                                st.success(
...
with tab_templates:
    ...
    with col1_tpl:
        ...
        selected_template_name_tpl = st.selectbox(
            "Seleziona Template", template_names, index=0, key="sb_template_tpl_st"
        )
        if selected_template_name_tpl:
            st.session_state.selected_template_id_tpl = templates_map[
                selected_template_name_tpl
            ]
            current_template_details = dr.get_kpi_indicator_template_by_id(
                st.session_state.selected_template_id_tpl
            )
        else:
            st.session_state.selected_template_id_tpl = None
        ...
        with st.expander("Gestisci Template", expanded=True):
            with st.form("template_form_tpl_st", clear_on_submit=False):
                tpl_form_key_prefix = (
                    f"tpl_form_st_{st.session_state.selected_template_id_tpl}_"
                    if st.session_state.selected_template_id_tpl
                    else "tpl_form_new_st_"
                )
                tpl_name = st.text_input(
                    "Nome Template",
                    value=(
                        current_template_details["name"]
                        if current_template_details
                        else ""
                    ),
                    key=tpl_form_key_prefix + "name",
                )
                tpl_desc = st.text_area(
                    "Descrizione",
                    value=(
                        current_template_details["description"]
                        if current_template_details
                        else ""
                    ),
                    height=100,
                    key=tpl_form_key_prefix + "desc",
                )
                col_add_tpl, col_save_tpl = st.columns(2)
                with col_add_tpl:
                    add_tpl_submitted = st.form_submit_button("Aggiungi Nuovo Template")
                with col_save_tpl:
                    save_tpl_submitted = st.form_submit_button(
                        "Salva Modifiche Template",
                        disabled=not st.session_state.selected_template_id_tpl,
                    )

                if add_tpl_submitted and tpl_name:
                    try:
                        kpi_templates_manager.add_kpi_indicator_template(
                            tpl_name, tpl_desc
                        )
                ...
                if (
                    save_tpl_submitted
                    and tpl_name
                    and st.session_state.selected_template_id_tpl
                ):
                    try:
                        kpi_templates_manager.update_kpi_indicator_template(
                            st.session_state.selected_template_id_tpl,
                            tpl_name,
                            tpl_desc,
                        )
            ...
            if st.session_state.selected_template_id_tpl:
                if st.button(
                    "Elimina Template Selezionato",
                    type="primary",
                    key=f"del_tpl_{st.session_state.selected_template_id_tpl}_st",
                ):
                    try:
                        kpi_templates_manager.delete_kpi_indicator_template(
                            st.session_state.selected_template_id_tpl
                        )
                        st.success(
                            f"Template '{selected_template_name_tpl}' eliminato."
                        )
                        st.session_state.selected_template_id_tpl = None
                        st.rerun()
                    except Exception as e:
        ...
        if st.button("Pulisci selezione template", key="clear_tpl_selection_st"):
            st.session_state.selected_template_id_tpl = None
            st.rerun()
    ...
    with col2_tpl:
        ...
        if st.session_state.selected_template_id_tpl:
            definitions = dr.get_template_defined_indicators(
                st.session_state.selected_template_id_tpl
            )
            if definitions:
                ...
                st.dataframe(
                    df_defs[
                        ...
                        columns={
                            "id": "ID",
                            "indicator_name_in_template": "Nome",
                            "default_calculation_type": "Tipo Calcolo",
                            "default_unit_of_measure": "Unità",
                            "default_visible": "Visibile",
                        }
                    ),
                    hide_index=True,
                )
                selected_def_id_for_action = st.selectbox(
                    "Seleziona definizione per Modifica/Elimina",
                    options=[""] + [d["id"] for d in definitions],
                    format_func=lambda x: (
                        next(
                            (
                                d["indicator_name_in_template"]
                ...
                if selected_def_id_for_action:
                    st.session_state.editing_definition_tpl = (
                        dr.get_template_indicator_definition_by_id(
                            selected_def_id_for_action
                        )
                    )
                    if st.button(
                        f"Elimina Definizione Selezionata ({st.session_state.editing_definition_tpl['indicator_name_in_template']})",
                        type="secondary",
                        key=f"del_def_{selected_def_id_for_action}_st",
                    ):
                        try:
                            kpi_templates_manager.remove_indicator_definition_from_template(
                                selected_def_id_for_action
                            )
            ...
            st.markdown("---")
            action_label = (
                "Modifica Definizione Indicatore"
                if st.session_state.editing_definition_tpl
                else "Aggiungi Nuova Definizione Indicatore"
            )
            st.subheader(action_label)

            def_data = st.session_state.editing_definition_tpl
            with st.form("definition_form_tpl_st", clear_on_submit=not bool(def_data)):
                def_form_key_prefix = (
                    f"def_form_st_{def_data['id']}_" if def_data else "def_form_new_st_"
                )
                def_name = st.text_input(
                    "Nome Indicatore nel Template",
                    value=def_data["indicator_name_in_template"] if def_data else "",
                    key=def_form_key_prefix + "name",
                )
                def_desc = st.text_area(
                    "Descrizione Default",
                    value=def_data["default_description"] if def_data else "",
                    height=70,
                    key=def_form_key_prefix + "desc",
                )
                def_calc_type = st.selectbox(
                    "Tipo Calcolo Default",
                    KPI_CALC_TYPE_OPTIONS,
                    index=(
                        KPI_CALC_TYPE_OPTIONS.index(
                            def_data["default_calculation_type"]
                        )
                        if def_data
                        and def_data["default_calculation_type"]
                        in KPI_CALC_TYPE_OPTIONS
                        else 0
                    ),
                    key=def_form_key_prefix + "calc",
                )
                def_unit = st.text_input(
                    "Unità Misura Default",
                    value=def_data["default_unit_of_measure"] if def_data else "",
                    key=def_form_key_prefix + "unit",
                )
                def_visible = st.checkbox(
                    "Visibile Default",
                    value=bool(def_data["default_visible"]) if def_data else True,
                    key=def_form_key_prefix + "vis",
                )
                submit_def_button = st.form_submit_button(
                    "Salva Definizione" if def_data else "Aggiungi Definizione"
                )
                if submit_def_button and def_name:
                    try:
                        if def_data:
                            ...
                            st.success(
                                f"Definizione '{def_name}' aggiunta al template."
                            )
                        st.session_state.editing_definition_tpl = None
                        st.rerun()
                    except Exception as e:
            ...
            st.info("Seleziona un template per gestire le sue definizioni.")
...
with tab_specs:
    ...
    with col_g_spec:
        selected_group_name_spec = st.selectbox(
            "Gruppo",
            group_names_spec,
            key="spec_group_sel_st",
            on_change=lambda: setattr(
                st.session_state, "spec_selected_subgroup_id", None
            )
            or setattr(st.session_state, "spec_selected_indicator_actual_id", None),
        )
        if selected_group_name_spec:
            st.session_state.spec_selected_group_id = groups_map_spec[
                selected_group_name_spec
            ]
        else:
            st.session_state.spec_selected_group_id = None

    subgroups_spec_list, subgroups_map_spec, subgroup_names_spec = [], {}, [""]
    if st.session_state.spec_selected_group_id:
        subgroups_spec_list = dr.get_kpi_subgroups_by_group_revised(
            st.session_state.spec_selected_group_id
        )
        for sg in subgroups_spec_list:
            subgroups_map_spec[sg["name"]] = sg["id"]
            subgroup_names_spec.append(sg["name"])

    with col_sg_spec:
        selected_subgroup_name_spec = st.selectbox(
            "Sottogruppo",
            subgroup_names_spec,
            key="spec_subgroup_sel_st",
        ...
        if selected_subgroup_name_spec:
            st.session_state.spec_selected_subgroup_id = subgroups_map_spec[
                selected_subgroup_name_spec
            ]
        else:
            st.session_state.spec_selected_subgroup_id = None

    indicators_spec_list, indicators_map_spec, indicator_names_spec = [], {}, [""]
    if st.session_state.spec_selected_subgroup_id:
        indicators_spec_list = dr.get_kpi_indicators_by_subgroup(
    ...
    with col_i_spec:
        selected_indicator_name_spec = st.selectbox(
            "Indicatore",
            indicator_names_spec,
            key="spec_indicator_sel_st",
            disabled=not st.session_state.spec_selected_subgroup_id,
        )
        if selected_indicator_name_spec:
            st.session_state.spec_selected_indicator_actual_id = indicators_map_spec[
                selected_indicator_name_spec
            ]
        else:
            st.session_state.spec_selected_indicator_actual_id = None

    st.markdown("---")
    current_kpi_spec_data, kpi_spec_id_for_form = None, None
    if st.session_state.spec_selected_indicator_actual_id:
        all_specs_for_lookup = dr.get_all_kpis_detailed()
        found_spec = next(
            (
                s
                for s in all_specs_for_lookup
                if s["actual_indicator_id"]
                == st.session_state.spec_selected_indicator_actual_id
            ),
            None,
        )
        if found_spec:
            kpi_spec_id_for_form = found_spec["id"]
            current_kpi_spec_data = found_spec
            st.session_state.current_editing_kpi_spec_id_specs = kpi_spec_id_for_form

    if (
        selected_indicator_name_spec
        and selected_subgroup_name_spec
        and selected_group_name_spec
    ):
        st.subheader(
            f"Specifica per: {selected_group_name_spec} > {selected_subgroup_name_spec} > {selected_indicator_name_spec}"
        )
    elif st.session_state.current_editing_kpi_spec_id_specs and current_kpi_spec_data:
        st.subheader(
            f"Modifica Specifica per: {get_kpi_display_name(current_kpi_spec_data)}"
        )

    with st.form("kpi_spec_form_st", clear_on_submit=False):
        spec_desc = st.text_area(
            "Descrizione Specifica",
            value=current_kpi_spec_data["description"] if current_kpi_spec_data else "",
        )
        spec_calc_type = st.selectbox(
            "Tipo Calcolo",
            KPI_CALC_TYPE_OPTIONS,
            index=(
                KPI_CALC_TYPE_OPTIONS.index(current_kpi_spec_data["calculation_type"])
                if current_kpi_spec_data
                and current_kpi_spec_data["calculation_type"] in KPI_CALC_TYPE_OPTIONS
                else 0
            ),
        )
        spec_unit = st.text_input(
            "Unità di Misura",
            value=(
                current_kpi_spec_data["unit_of_measure"]
                if current_kpi_spec_data
                else ""
            ),
        )
        spec_visible = st.checkbox(
            "Visibile",
            value=(
                bool(current_kpi_spec_data["visible"])
                if current_kpi_spec_data
                else True
            ),
        )
        ...
        if submit_spec_button:
            if (
                not st.session_state.spec_selected_indicator_actual_id
                and not st.session_state.current_editing_kpi_spec_id_specs
            ):
                st.error("Seleziona un indicatore prima di salvare.")
            else:
                try:
                    indicator_id_to_use = (
                        st.session_state.spec_selected_indicator_actual_id
                    ...
                    if (
                        st.session_state.current_editing_kpi_spec_id_specs
                        and current_kpi_spec_data
                    ):
                        indicator_id_to_use = current_kpi_spec_data[
                            "actual_indicator_id"
                        ]

                    if (
                        kpi_spec_id_for_form
                        or st.session_state.current_editing_kpi_spec_id_specs
                    ):
                        spec_id_to_update = (
                            kpi_spec_id_for_form
                            or st.session_state.current_editing_kpi_spec_id_specs
                        ...
                        st.success("Specifiche KPI aggiornate.")
                    else:
                        ...
                        st.success("Specifiche KPI aggiunte.")
                    st.session_state.current_editing_kpi_spec_id_specs = None
                    st.rerun()
                except Exception as e:
                    st.error(f"Errore salvataggio specifiche: {e}")
                    st.error(traceback.format_exc())

    if (
        st.session_state.current_editing_kpi_spec_id_specs
        or st.session_state.spec_selected_indicator_actual_id
    ):
        if st.button("Pulisci selezione", key="clear_spec_form_button_st"):
            (
                st.session_state.spec_selected_group_id,
                st.session_state.spec_selected_subgroup_id,
                st.session_state.spec_selected_indicator_actual_id,
    ...
    if all_kpi_specs_data:
        ...
        df_all_specs["Nome Completo KPI"] = df_all_specs.apply(
            lambda row: get_kpi_display_name(row.to_dict()), axis=1
        )
        cols_to_display = [
            "id",
            "Nome Completo KPI",
        ...
        df_display_specs = df_all_specs[cols_to_display].rename(
            columns={
                "id": "ID Spec",
        ...
        df_display_specs["Visibile"] = df_display_specs["Visibile"].apply(
            lambda x: "Sì" if x else "No"
        )
        selected_spec_from_table = st.dataframe(
            df_display_specs,
            hide_index=True,
            on_select="rerun",
            selection_mode="single-row",
            key="spec_table_selection_st",
        )
        spec_table_selection_state = st.session_state.get("spec_table_selection_st")
        if (
            spec_table_selection_state
            and spec_table_selection_state["selection"]["rows"]
        ):
...
with tab_links:
    ...
    all_links_raw = dr.get_all_master_sub_kpi_links()
    kpi_spec_map_link = {}

    if all_kpis_for_link_raw:
        for row_obj in all_kpis_for_link_raw:
            if row_obj and row_obj["id"] is not None:
                display_name = get_kpi_display_name(dict(row_obj))
                if not display_name.startswith("N/D"):
                    kpi_spec_map_link[row_obj["id"]] = display_name
    else:
        st.info(
            "Nessun KPI definito. Aggiungere KPI e Specifiche nelle rispettive schede."
        )

    # --- UI Components ---
    st.subheader("Seleziona KPI per Gestire Link")

    sorted_kpi_items = sorted(kpi_spec_map_link.items(), key=lambda item: item[1])
    kpi_spec_ids_link_options = [""] + [item[0] for item in sorted_kpi_items]

    selected_kpi_id = st.selectbox(
        "KPI Principale",
        options=kpi_spec_ids_link_options,
        format_func=lambda k_id: kpi_spec_map_link.get(k_id, "Seleziona KPI..."),
    )

    # --- Display KPI Role and Links ---
    if selected_kpi_id:
        role_details = dr.get_kpi_role_details(selected_kpi_id)

        st.write(f"**KPI Selezionato:** {kpi_spec_map_link.get(selected_kpi_id)}")
        st.write(
            f"**Ruolo Attuale:** {role_details['role'].upper() if role_details['role'] else 'Nessuno'}"
        )
        ...
        if role_details["role"] == "master":
            ...
            linked_subs_data = [
                {
                    "ID SubKPI": link["sub_kpi_spec_id"],
                    "Nome SubKPI": kpi_spec_map_link.get(
                        link["sub_kpi_spec_id"], "N/A"
                    ),
                    "Peso Distribuzione": link["distribution_weight"],
                }
                for link in all_links_raw
                if link["master_kpi_spec_id"] == selected_kpi_id
            ]
            if linked_subs_data:
                st.dataframe(pd.DataFrame(linked_subs_data), hide_index=True)
            else:
                st.info("Nessun Sub-KPI attualmente collegato a questo Master.")

        elif role_details["role"] == "sub" and role_details.get("master_id"):
            master_id = role_details["master_id"]
            link_info = next(
                (
                    link
                    for link in all_links_raw
                    if link["master_kpi_spec_id"] == master_id
                    and link["sub_kpi_spec_id"] == selected_kpi_id
                ),
                None,
            )
            weight = link_info["distribution_weight"] if link_info else 1.0
            st.markdown(
                f"#### È Sub-KPI di: **{kpi_spec_map_link.get(master_id)}** (Peso: {weight})"
            )
        ...
        with link_col1:
            with st.form("link_sub_form_st", clear_on_submit=True):
                st.markdown("**Collega un Sub-KPI a questo KPI (Master):**")

                # --- FIX: Simplified and Corrected Filtering for Sub-KPI candidates ---
                all_master_ids = {link["master_kpi_spec_id"] for link in all_links_raw}
                all_sub_ids = {link["sub_kpi_spec_id"] for link in all_links_raw}

                available_subs_map = {
                    kpi_id: name
                    for kpi_id, name in kpi_spec_map_link.items()
                    if kpi_id != selected_kpi_id
                    and kpi_id not in all_sub_ids
                    and kpi_id not in all_master_ids
                }
                # --- END OF FIX ---

                sub_to_link_id = st.selectbox(
                    "Sub-KPI da Collegare",
                    options=[""] + list(available_subs_map.keys()),
                    format_func=lambda s_id: available_subs_map.get(
                        s_id, "Seleziona..."
                    ),
                )
                link_weight = st.number_input(
                    "Peso Distribuzione", min_value=0.01, value=1.0, step=0.1
                )

                if st.form_submit_button("🔗 Collega Sub-KPI"):
                    if sub_to_link_id:
                        try:
                            kpi_links_manager.add_master_sub_kpi_link(
                                selected_kpi_id, sub_to_link_id, link_weight
                            )
                            st.success("Sub-KPI collegato.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Errore: {e}")
                    else:
                        st.warning("Selezionare un Sub-KPI da collegare.")

        with link_col2:
            if role_details["role"] == "master":
                with st.form("unlink_sub_form_st", clear_on_submit=True):
                    st.markdown("**Scollega un Sub-KPI:**")
                    subs_of_master_map = {
                        link["sub_kpi_spec_id"]: kpi_spec_map_link.get(
                            link["sub_kpi_spec_id"]
                        )
                        for link in all_links_raw
                        if link["master_kpi_spec_id"] == selected_kpi_id
                    }
                    sub_to_unlink_id = st.selectbox(
                        "Sub-KPI da Scollegare",
                        options=[""] + list(subs_of_master_map.keys()),
                        format_func=lambda s_id: subs_of_master_map.get(
                            s_id, "Seleziona..."
                        ),
                    )
                    if st.form_submit_button("🗑️ Scollega Sub-KPI"):
                        if sub_to_unlink_id:
                            try:
                                kpi_links_manager.remove_master_sub_kpi_link(
                                    selected_kpi_id, sub_to_unlink_id
                                )
                                st.success("Scollegamento eseguito.")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Errore: {e}")
                        else:
                            st.warning("Selezionare un Sub-KPI da scollegare.")
            elif role_details["role"] == "sub" and role_details.get("master_id"):
                if st.button(
                    f"🗑️ Scollega da Master ({kpi_spec_map_link.get(role_details['master_id'])})",
                    type="secondary",
                ):
                    try:
                        kpi_links_manager.remove_master_sub_kpi_link(
                            role_details["master_id"], selected_kpi_id
                        )
                        st.success("Scollegamento eseguito.")
                        st.rerun()
                    except Exception as e:
        ...
        with link_col3:
            if role_details["role"] == "master":
                with st.form("update_weight_form_st", clear_on_submit=False):
                    st.markdown("**Modifica Peso:**")
                    subs_of_master_map = {
                        link["sub_kpi_spec_id"]: kpi_spec_map_link.get(
                            link["sub_kpi_spec_id"]
                        )
                    ...
                    if sub_to_update_id:
                        link_info = next(
                            (
                                ...
                                for link in all_links_raw
                                if link["master_kpi_spec_id"] == selected_kpi_id
                                and link["sub_kpi_spec_id"] == sub_to_update_id
                            ),
                            None,
                        )
                        if link_info:
                            current_weight = link_info["distribution_weight"]
                    ...
                    if st.form_submit_button("⚖️ Aggiorna Peso"):
                        if sub_to_update_id:
                            try:
                                kpi_links_manager.update_master_sub_kpi_link_weight(
                                    selected_kpi_id, sub_to_update_id, new_weight
                                )
                            ...
                            st.warning("Selezionare un Sub-KPI per aggiornare il peso.")
    else:
        st.info("Seleziona un KPI Principale per gestire i suoi link.")
...
with tab_target:
    ...
    # This dictionary is populated by load_kpi_data_for_target_entry()
    # and is used to render the filters.
    if "stabilimenti_map_target_ui" not in globals():
        try:
            stabilimenti_all_for_map = dr.get_all_stabilimenti(only_visible=True)
            globals()["stabilimenti_map_target_ui"] = {
                s["name"]: s["id"] for s in stabilimenti_all_for_map
            }
        except Exception as e:
    ...
    with target_filter_cols[0]:
        ...
        current_year = st.session_state.get(
            "target_year_sb_filters", str(datetime.datetime.now().year)
        )
        st.selectbox(
            "Anno",
    ...
    with target_filter_cols[1]:
        stab_names = [""] + list(stabilimenti_map_target_ui.keys())
        current_stab = st.session_state.get("target_stab_sb_filters")
        st.selectbox(
            "Stabilimento",
            stab_names,
            index=stab_names.index(current_stab) if current_stab in stab_names else 0,
    ...
    # --- Data Loading Trigger Logic ---
    if "last_loaded_year_target_tab" not in st.session_state:
        st.session_state.last_loaded_year_target_tab = None
    if "last_loaded_stab_target_tab" not in st.session_state:
        st.session_state.last_loaded_stab_target_tab = None

    year_filter_val_check = st.session_state.get("target_year_sb_filters")
    stab_filter_val_check = st.session_state.get("target_stab_sb_filters")
    if (
        year_filter_val_check
        and stab_filter_val_check
        and (
            st.session_state.last_loaded_year_target_tab != year_filter_val_check
            or st.session_state.last_loaded_stab_target_tab != stab_filter_val_check
            or not st.session_state.get("kpi_target_inputs")
        )
    ):
        load_kpi_data_for_target_entry()
        st.session_state.last_loaded_year_target_tab = year_filter_val_check
        st.session_state.last_loaded_stab_target_tab = stab_filter_val_check
        st.rerun()

    # --- Display KPI Target Entry Fields ---
    year_str_check = st.session_state.get("target_year_sb_filters")
    stab_name_check = st.session_state.get("target_stab_sb_filters")
    kpi_inputs = st.session_state.get("kpi_target_inputs")

    if not (year_str_check and stab_name_check and kpi_inputs):
        st.info("Seleziona Anno e Stabilimento per visualizzare o inserire i target.")
    else:
        year_disp = year_str_check
        stab_disp = stab_name_check
        stab_id_key = stabilimenti_map_target_ui.get(stab_disp, "")
        key_suffix = f"_{year_disp}_{stab_id_key}"

        st.markdown("---")
        st.subheader(f"Target per {stab_disp} - Anno {year_disp}")

        sorted_kpi_ids = sorted(
            kpi_inputs.keys(), key=lambda k: kpi_inputs[k].get("display_name", str(k))
        )

        for kpi_spec_id in sorted_kpi_ids:
            kpi_session_data = kpi_inputs[kpi_spec_id]
            key_base = f"kpi_entry_{kpi_spec_id}"

            use_formula1_widget_val = st.session_state.get(
                f"{key_base}_use_formula1{key_suffix}",
                kpi_session_data.get("target1_is_formula_based", False),
            )
            use_formula2_widget_val = st.session_state.get(
                f"{key_base}_use_formula2{key_suffix}",
                kpi_session_data.get("target2_is_formula_based", False),
            )

            is_manual1_widget_val = (
                use_formula1_widget_val is False
                and st.session_state.get(
                    f"{key_base}_is_manual1{key_suffix}",
                    kpi_session_data.get("is_manual1", True),
                )
            )
            is_manual2_widget_val = (
                use_formula2_widget_val is False
                and st.session_state.get(
                    f"{key_base}_is_manual2{key_suffix}",
                    kpi_session_data.get("is_manual2", True),
                )
            )

            exp_label_prefix = ""
            if kpi_session_data.get("is_sub_kpi"):
                if use_formula1_widget_val or use_formula2_widget_val:
                    exp_label_prefix = "🧪 (Formula) "
                elif not is_manual1_widget_val or not is_manual2_widget_val:
                    exp_label_prefix = "⚙️ (Derivato) "
                else:
                    exp_label_prefix = "✏️ (Manuale) "
            elif use_formula1_widget_val or use_formula2_widget_val:
                exp_label_prefix = "🧪 (Formula) "
            else:
                exp_label_prefix = "✏️ (Manuale Def.) "

            exp_label = f"{exp_label_prefix}{kpi_session_data.get('display_name', 'KPI N/D')} (ID Spec: {kpi_spec_id})"

            with st.expander(exp_label):
                st.markdown("###### Target 1")
                t1_row = st.columns([0.3, 0.15, 0.25, 0.9, 0.3])
                with t1_row[0]:
                    st.number_input(
                        "Valore T1",
                        value=st.session_state.get(
                            f"{key_base}_target1{key_suffix}",
                            kpi_session_data.get("target1", 0.0),
                        ),
                        key=f"{key_base}_target1{key_suffix}",
                        format="%.2f",
                        disabled=use_formula1_widget_val
                        or (
                            kpi_session_data.get("is_sub_kpi")
                            and not is_manual1_widget_val
                        ),
                        on_change=(
                            on_master_target_ui_change_st
                ...
                if kpi_session_data.get("is_sub_kpi"):
                    with t1_row[1]:
                        st.checkbox(
                            "Man. T1",
                            value=is_manual1_widget_val,
                            key=f"{key_base}_is_manual1{key_suffix}",
                            on_change=on_sub_manual_flag_ui_change_st,
                            args=(kpi_spec_id, "1", key_suffix),
                            disabled=use_formula1_widget_val,
                        )
                with t1_row[2]:
                    st.checkbox(
                        "Formula T1",
                        value=use_formula1_widget_val,
                        key=f"{key_base}_use_formula1{key_suffix}",
                        on_change=_on_use_formula_toggle_st,
                        args=(kpi_spec_id, "1", key_suffix),
                    )
                with t1_row[3]:
                    st.text_input(
                        "Def. Formula T1",
                        value=st.session_state.get(
                            f"{key_base}_formula_str1{key_suffix}",
                            kpi_session_data.get("target1_formula", ""),
                        ),
                        key=f"{key_base}_formula_str1{key_suffix}",
                        disabled=not use_formula1_widget_val,
                        placeholder="Es: var_A * 0.5",
                        label_visibility="collapsed",
                    )
                with t1_row[4]:
                    if st.button(
                        "Input F.T1",
                        key=f"{key_base}_btn_formula_inputs1{key_suffix}",
                        disabled=not use_formula1_widget_val,
                        use_container_width=True,
                    ):
                        st.session_state.formula_input_dialog_open_for = (
                            f"{kpi_spec_id}_1{key_suffix}"
                        )
                        st.session_state.current_formula_inputs_temp = json.loads(
                            kpi_session_data.get("target1_formula_inputs_json", "[]")
                        )
                        st.rerun()
                ...
                with t2_row[0]:
                    st.number_input(
                        "Valore T2",
                        value=st.session_state.get(
                            f"{key_base}_target2{key_suffix}",
                            kpi_session_data.get("target2", 0.0),
                        ),
                        key=f"{key_base}_target2{key_suffix}",
                        format="%.2f",
                        disabled=use_formula2_widget_val
                        or (
                            kpi_session_data.get("is_sub_kpi")
                            and not is_manual2_widget_val
                        ),
                        on_change=(
                            on_master_target_ui_change_st
                            if kpi_session_data.get("is_master_kpi")
                ...
                if kpi_session_data.get("is_sub_kpi"):
                    with t2_row[1]:
                        st.checkbox(
                            "Man. T2",
                            value=is_manual2_widget_val,
                            key=f"{key_base}_is_manual2{key_suffix}",
                            on_change=on_sub_manual_flag_ui_change_st,
                            args=(kpi_spec_id, "2", key_suffix),
                            disabled=use_formula2_widget_val,
                        )
                with t2_row[2]:
                    st.checkbox(
                        "Formula T2",
                        value=use_formula2_widget_val,
                        key=f"{key_base}_use_formula2{key_suffix}",
                        on_change=_on_use_formula_toggle_st,
                        args=(kpi_spec_id, "2", key_suffix),
                    )
                with t2_row[3]:
                    st.text_input(
                        "Def. Formula T2",
                        value=st.session_state.get(
                            f"{key_base}_formula_str2{key_suffix}",
                            kpi_session_data.get("target2_formula", ""),
                        ),
                        key=f"{key_base}_formula_str2{key_suffix}",
                        disabled=not use_formula2_widget_val,
                        placeholder="Es: var_X + var_Y",
                        label_visibility="collapsed",
                    )
                with t2_row[4]:
                    if st.button(
                        "Input F.T2",
                        key=f"{key_base}_btn_formula_inputs2{key_suffix}",
                        disabled=not use_formula2_widget_val,
                        use_container_width=True,
                    ):
                        st.session_state.formula_input_dialog_open_for = (
                            f"{kpi_spec_id}_2{key_suffix}"
                        )
                        st.session_state.current_formula_inputs_temp = json.loads(
                            kpi_session_data.get("target2_formula_inputs_json", "[]")
                        )
                        st.rerun()
                ...
                st.selectbox(
                    ...
                    index=profile_idx,
                    key=f"{key_base}_profile{key_suffix}",
                )

                current_profile_ui = st.session_state.get(
                    f"{key_base}_profile{key_suffix}", profile_val_from_backing
                )
                show_logic_radios_ui = not (
                    current_profile_ui
                    in [
                        PROFILE_ANNUAL_PROGRESSIVE,
                        PROFILE_ANNUAL_PROGRESSIVE_WEEKDAY_BIAS,
                ...
                if show_logic_radios_ui:
                    st.selectbox(
                        ...
                        index=logic_idx,
                        key=f"{key_base}_logic{key_suffix}",
                    )

                effective_logic_for_display = (
                    st.session_state.get(
                        f"{key_base}_logic{key_suffix}", logic_val_from_backing
                    )
                    if show_logic_radios_ui
                ...
                if (
                    ...
                    for i, month_name in enumerate(months):
                        with month_cols[i % 4]:
                            default_val = raw_repart_db_vals.get(
                                month_name,
                                (
                                    round(100 / 12, 2)
                            ...
                            st.number_input(
                                month_name[:3],
                                value=st.session_state.get(
                                    f"{key_base}_month_{i}{key_suffix}", default_val
                                ),
                                format="%.2f",
                                key=f"{key_base}_month_{i}{key_suffix}",
                            )
                elif (
                    ...
                    for i, q_name in enumerate(quarters):
                        with q_cols[i % 4]:
                            ...
                            st.number_input(
                                q_name,
                                value=st.session_state.get(
                                    f"{key_base}_q_{i}{key_suffix}", default_val
                                ),
                                format="%.2f",
                                key=f"{key_base}_q_{i}{key_suffix}",
                            )
                elif (
                    ...
                    st.text_area(
                        "JSON Settimanale",
                        value=st.session_state.get(
                            f"{key_base}_weekly_json{key_suffix}", default_val
                        ),
                        height=100,
                        key=f"{key_base}_weekly_json{key_suffix}",
                    )
                ...
                if current_profile_ui == "event_based_spikes_or_dips":
                    ...
                    st.text_area(
                        "JSON Eventi",
                        value=st.session_state.get(
                            f"{key_base}_event_json{key_suffix}", default_val
                        ),
                        height=120,
                        key=f"{key_base}_event_json{key_suffix}",
                    )

        if st.session_state.get("formula_input_dialog_open_for"):
            dialog_full_key = st.session_state.formula_input_dialog_open_for
            dialog_kpi_id_str, dialog_target_num_str, *dialog_suffix_parts = (
                dialog_full_key.split("_")
            )
            ...
            if dialog_kpi_data:
                with st.container(border=True):
                    st.subheader(
                        f"Definisci Input Formula per {dialog_kpi_data.get('display_name')} - Target {dialog_target_num}"
                    )
                    kpi_options_for_dialog = st.session_state.get(
                        "all_kpis_for_formula_selection_cache_st", {}
                    )
                    filtered_kpi_options = {
                        kid: name
                        for kid, name in kpi_options_for_dialog.items()
                        if kid != dialog_kpi_id
                    }

                    modal_sel_kpi_key = (
                        f"temp_modal_kpi_sel_{dialog_kpi_id}_{dialog_target_num}"
                    )
                    modal_target_source_key = f"temp_modal_target_source_sel_{dialog_kpi_id}_{dialog_target_num}"
                    modal_var_name_key = (
                        f"temp_modal_var_name_{dialog_kpi_id}_{dialog_target_num}"
                    )

                    selected_input_kpi_id_val = st.selectbox(
                        "Seleziona KPI Input",
                        options=[""] + list(filtered_kpi_options.keys()),
                        format_func=lambda kid: (
                            filtered_kpi_options.get(kid, "Seleziona...")
                            if kid
                            else "Seleziona..."
                        ),
                        key=modal_sel_kpi_key,
                    )
                    target_source_sel_val = st.selectbox(
                        "Sorgente Target del KPI Input",
                        options=["annual_target1", "annual_target2"],
                        key=modal_target_source_key,
                    )
                    variable_name_formula_val = (
                        st.text_input(
                            "Nome Variabile (in formula)",
                            key=modal_var_name_key,
                            placeholder="Es: ValoreDaKPI_A",
                        )
                        .strip()
                        .replace(" ", "_")
                    )
                    ...
                    if st.button(
                        "➕ Aggiungi Input",
                        key=f"formula_modal_add_btn_{dialog_kpi_id}_{dialog_target_num}",
                    ):
                        if (
                            selected_input_kpi_id_val
                            and target_source_sel_val
                            and variable_name_formula_val
                        ):
                            if not variable_name_formula_val.isidentifier():
                                st.warning("Nome variabile non valido.")
                            elif any(
                                item["variable_name"] == variable_name_formula_val
                                for item in st.session_state.current_formula_inputs_temp
                            ):
                                st.warning(
                                    f"Nome variabile '{variable_name_formula_val}' già in uso."
                                )
                            else:
                                st.session_state.current_formula_inputs_temp.append(
                                    {
                                        "kpi_id": selected_input_kpi_id_val,
                                        "target_source": target_source_sel_val,
                                        "variable_name": variable_name_formula_val,
                                    }
                                )
                                st.rerun()
                        else:
                            st.warning("Completa tutti i campi per l'input.")
                    ...
                    if st.session_state.current_formula_inputs_temp:
                        for i, item in enumerate(
                            ...
                            kpi_name_disp = kpi_options_for_dialog.get(
                                item["kpi_id"], f"ID: {item['kpi_id']}"
                            )
                            st.markdown(
                                f"`{i+1}`: `{item['variable_name']}` = *{kpi_name_disp}*.`{item['target_source']}`"
                            )

                        idx_to_remove_options = [""] + list(
                            range(len(st.session_state.current_formula_inputs_temp))
                        )
                        idx_to_remove_val = st.selectbox(
                            "Rimuovi Input N.",
                            options=idx_to_remove_options,
                            format_func=lambda x: (
                                f"Input {x+1}" if isinstance(x, int) else "Seleziona..."
                            ),
                            key=f"formula_modal_remove_sel_{dialog_kpi_id}_{dialog_target_num}",
                        )
                        if st.button(
                            "➖ Rimuovi Selezionato",
                            key=f"formula_modal_remove_btn_{dialog_kpi_id}_{dialog_target_num}",
                        ):
                            if isinstance(
                                idx_to_remove_val, int
                            ) and 0 <= idx_to_remove_val < len(
                                st.session_state.current_formula_inputs_temp
                            ):
                                st.session_state.current_formula_inputs_temp.pop(
                                    idx_to_remove_val
                                )
                                st.rerun()
                    else:
                        st.caption("Nessun input definito.")

                    st.markdown("---")
                    modal_cols = st.columns(2)
                    with modal_cols[0]:
                        if st.button(
                            "✅ Conferma Input Formula",
                            key=f"formula_modal_ok_btn_{dialog_kpi_id}_{dialog_target_num}",
                            type="primary",
                            use_container_width=True,
                        ):
                            ...
                            st.session_state.kpi_target_inputs[dialog_kpi_id][
                                f"target{dialog_target_num}_formula_inputs_json"
                            ] = inputs_json_str
                            st.session_state.formula_input_dialog_open_for = None
                            st.session_state.current_formula_inputs_temp = []
                            st.rerun()
                    with modal_cols[1]:
                        if st.button(
                            "❌ Annulla",
                            key=f"formula_modal_cancel_btn_{dialog_kpi_id}_{dialog_target_num}",
                            use_container_width=True,
                        ):
                            st.session_state.formula_input_dialog_open_for = None
                            st.session_state.current_formula_inputs_temp = []
        ...
        st.markdown("---")
        # --- FIX: Complete Save Button Logic ---
        if st.button(
            "SALVA TUTTI I TARGET",
            type="primary",
            use_container_width=True,
            key="save_all_targets_button_main_st",
        ):
            year_to_save_val = int(st.session_state.target_year_sb_filters)
            stab_id_to_save_val = stabilimenti_map_target_ui[
                st.session_state.target_stab_sb_filters
            ]
            ...
            for (
                ...
                try:
                    t1_val_save = st.session_state.get(
                        f"{key_base_save}_target1{current_key_suffix_save}",
                        kpi_data_sess_save.get("target1", 0.0),
                    )
                    t2_val_save = st.session_state.get(
                        f"{key_base_save}_target2{current_key_suffix_save}",
                        kpi_data_sess_save.get("target2", 0.0),
                    )
                except KeyError:
                    st.error(
                        f"Chiave widget mancante per KPI {kpi_id_save}. Impossibile salvare."
                    )
                    all_inputs_valid_save = False
                    break

                profile_save = st.session_state.get(
                    f"{key_base_save}_profile{current_key_suffix_save}",
                    kpi_data_sess_save.get("profile"),
                )

                t1_use_formula_val_save = st.session_state.get(
                    f"{key_base_save}_use_formula1{current_key_suffix_save}",
                    kpi_data_sess_save.get("target1_is_formula_based", False),
                )
                t1_formula_str_save = (
                    st.session_state.get(
                        f"{key_base_save}_formula_str1{current_key_suffix_save}",
                        kpi_data_sess_save.get("target1_formula", ""),
                    )
                    if t1_use_formula_val_save
                    else None
                )
                t1_formula_inputs_json_save = kpi_data_sess_save.get(
                    "target1_formula_inputs_json", "[]"
                )
                try:
                    t1_formula_inputs_py_save = json.loads(t1_formula_inputs_json_save)
                except json.JSONDecodeError:
                    st.error(f"JSON T1 Input Formula corrotto per {kpi_id_save}")
                    all_inputs_valid_save = False
                    break

                t2_use_formula_val_save = st.session_state.get(
                    f"{key_base_save}_use_formula2{current_key_suffix_save}",
                    kpi_data_sess_save.get("target2_is_formula_based", False),
                )
                t2_formula_str_save = (
                    st.session_state.get(
                        f"{key_base_save}_formula_str2{current_key_suffix_save}",
                        kpi_data_sess_save.get("target2_formula", ""),
                    )
                    if t2_use_formula_val_save
                    else None
                )
                t2_formula_inputs_json_save = kpi_data_sess_save.get(
                    "target2_formula_inputs_json", "[]"
                )
                try:
                    t2_formula_inputs_py_save = json.loads(t2_formula_inputs_json_save)
                except json.JSONDecodeError:
                    st.error(f"JSON T2 Input Formula corrotto per {kpi_id_save}")
                    all_inputs_valid_save = False
                    break

                is_manual1_final_save = (
                    False
                    if t1_use_formula_val_save
                    else st.session_state.get(
                        f"{key_base_save}_is_manual1{current_key_suffix_save}",
                        kpi_data_sess_save.get("is_manual1", True),
                    )
                )
                is_manual2_final_save = (
                    False
                    if t2_use_formula_val_save
                    else st.session_state.get(
                        f"{key_base_save}_is_manual2{current_key_suffix_save}",
                        kpi_data_sess_save.get("is_manual2", True),
                    )
                )

                repart_values_for_db_save, profile_params_for_db_save = {}, {}
                show_logic_radios_save_check = not (
                    profile_save
                    in [
                        PROFILE_ANNUAL_PROGRESSIVE,
                        PROFILE_ANNUAL_PROGRESSIVE_WEEKDAY_BIAS,
                ...
                effective_logic_db_save = (
                    st.session_state.get(
                        f"{key_base_save}_logic{current_key_suffix_save}",
                        kpi_data_sess_save.get("logic"),
                    )
                ...
                if (
                    ...
                    months_save = [
                        calendar.month_name[i_m_save] for i_m_save in range(1, 13)
                    ]
                    current_sum_percent = 0.0
                    for i_m, month_n_save in enumerate(months_save):
                        val_m = st.session_state.get(
                            f"{key_base_save}_month_{i_m}{current_key_suffix_save}", 0.0
                        )
                        repart_values_for_db_save[month_n_save] = float(val_m)
                        current_sum_percent += float(val_m)
                    if (
                        kpi_data_sess_save.get("calc_type") == CALC_TYPE_INCREMENTALE
                        and (
                    ...
                    for i_q, q_n_save in enumerate(quarters_save):
                        val_q = st.session_state.get(
                            f"{key_base_save}_q_{i_q}{current_key_suffix_save}", 0.0
                        )
                    ...
                    json_str_weekly_save = st.session_state.get(
                        f"{key_base_save}_weekly_json{current_key_suffix_save}", "{}"
                    )
                    try:
                        repart_values_for_db_save = json.loads(json_str_weekly_save)
                    except json.JSONDecodeError:
                        st.error(f"JSON settimanale non valido per KPI {kpi_id_save}")
                        all_inputs_valid_save = False
                        break

                if profile_save == "event_based_spikes_or_dips":
                    json_str_event_save = st.session_state.get(
                        f"{key_base_save}_event_json{current_key_suffix_save}", "[]"
                    )
                    try:
                        ...
                        st.error(f"JSON eventi non valido per KPI {kpi_id_save}")
                        all_inputs_valid_save = False
                        break
                ...
                targets_to_save_for_db[str(kpi_id_save)] = {
                    ...
                    "is_target1_manual": is_manual1_final_save,
                    "is_manual2": is_manual2_final_save,
                    "target1_is_formula_based": t1_use_formula_val_save,
                    "target1_formula": t1_formula_str_save,
                    "target1_formula_inputs": t1_formula_inputs_py_save,
                    "target2_is_formula_based": t2_use_formula_val_save,
                    "target2_formula": t2_formula_str_save,
            ...
            if all_inputs_valid_save and targets_to_save_for_db:
                try:
                    annual_targets_manager.save_annual_targets(
                        year_to_save_val, stab_id_to_save_val, targets_to_save_for_db
                    )
        ...
        for stab_row in all_stabilimenti_dash:
            ...
            with st.expander(f"🏢 Stabilimento: {stabilimento_name}", expanded=False):
                ...
                for kpi_spec_row_sqlite in all_kpis_dash:
                    ...
                    kpi_display_name = get_kpi_display_name(kpi_spec_row)
